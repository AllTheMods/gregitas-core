...
public class PressurePipeNet extends PipeNet<PressurePipeData> implements IPressureContainer {
    ...
    private double netParticles = GregitasConstants.EARTH_PRESSURE;
    ...
    public void onLeak() {
        if (getPressure() < GregitasConstants.EARTH_PRESSURE) changeParticles(getLeakRate(), false);
        else if (getPressure() > GregitasConstants.EARTH_PRESSURE) changeParticles(-getLeakRate(), false);
    }
